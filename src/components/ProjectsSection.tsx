...
export default function projectsSection() {
  ...
  return (
    <SectionLayout
      id="projects"
      ref={mainRef}
      className="bg-secondary/30 m-4 inset-0 rounded-md border border-foreground/10 "
    >
      <div className="max-w-7xl mx-auto md:pt-30 md:pb-20 lg:pb-30">
        ...
        "
          ...
          {/* línea decorativa */}
          <div className="gsap-reveal h-[0.75] w-24 bg-killua/80 rounded-full"></div>

          <p className="gsap-reveal text-xl text-muted-foreground max-w-2xl leading-relaxed">
        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {projects.map((project, index) => (
            <ProjectCard
              ...
              project={project}
              index={index}
              projectImageAlt={t.projects.projectImageAlt}
            />
          ))}
        </div>
      </div>
